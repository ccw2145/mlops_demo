...
The databricks CLI bundles top level is defined by file `azure_stacks_demo/databricks.yml`.
...
ML Resource Configurations in this directory:
 - model workflow (`azure_stacks_demo/assets/model-workflow-asset.yml`)
 - batch inference workflow (`azure_stacks_demo/assets/batch-inference-workflow-asset.yml`)
 - monitoring workflow (`azure_stacks_demo/assets/monitoring-workflow-asset.yml`)
 - feature engineering workflow (`azure_stacks_demo/assets/feature-engineering-workflow-asset.yml`)
 - model definition and experiment definition (`azure_stacks_demo/assets/ml-artifacts-asset.yml`)
...
If you want to use different catalog names, please update the `targets` declared in the `azure_stacks_demo/databricks.yml` and `azure_stacks_demo/assets/ml-artifacts-asset.yml` files.
...
|-------------|---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|----------------------|-------------------------------------|------------------------------------------------|
| dev         | The `dev` deployment target is used by ML engineers to deploy ML assets to development workspace with `dev` configs. The config is for ML project development purposes.                                                           | dev workspace        | dev-azure_stacks_demo-model     | /dev-azure_stacks_demo-experiment     |
| staging     | The `staging` deployment target is part of the CD pipeline. Latest main content will be deployed to staging workspace with `staging` config.                                                             | staging workspace    | staging-azure_stacks_demo-model | /staging-azure_stacks_demo-experiment |
| prod        | The `prod` deployment target is part of the CD pipeline. Latest release content will be deployed to prod workspace with `prod` config.                                                                      | prod workspace       | prod-azure_stacks_demo-model    | /prod-azure_stacks_demo-experiment    |
| test        | The `test` deployment target is part of the CI pipeline. For changes targeting the main branch, upon making a PR, an integration test will be triggered and ML assets deployed to the staging workspace defined under `test` deployment target. | staging workspace    | test-azure_stacks_demo-model    | /test-azure_stacks_demo-experiment    |
...
2. Complete the `TODO` in `azure_stacks_demo/databricks.yml` to add the dev workspace URI under `targets.dev.workspace.host`.
...
Following this, update the batch_inference_job base parameters in `azure_stacks_demo/assets/batch-inference-workflow-asset.yml` to pass
...
To get started, open `azure_stacks_demo/assets/batch-inference-workflow-asset.yml`.  The file contains the ML asset definition of a batch inference job, like:
...
resources:
  jobs:
    batch_inference_job:
      name: ${bundle.target}-azure_stacks_demo-batch-inference-job
      tasks:
...
The example above defines a Databricks job with name `${bundle.target}-azure_stacks_demo-batch-inference-job`
that runs the notebook under `azure_stacks_demo/deployment/batch_inference/notebooks/BatchInference.py` to regularly apply your ML model for batch inference. 
...
We specify a `batch_inference_job` under `assets/jobs` to define a databricks workflow with internal key `batch_inference_job` and job name `{bundle.target}-azure_stacks_demo-batch-inference-job`.
...
`test-azure_stacks_demo-batch-inference-job`. During the deployment of the `staging` environment config, the job name will be
`staging-azure_stacks_demo-batch-inference-job`.
